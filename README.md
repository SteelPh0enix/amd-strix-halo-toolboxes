...
| `rocm-10.0-strix-llama` | ROCm 10.0 (Experimental) | Builds [`halo-box/strix-llama.cpp:master`](https://github.com/halo-box/strix-llama.cpp) against a **retained-PM4 ROCr + HIP** runtime compiled from [`pwilkin/rocm-systems:ilintar-experiments`](https://github.com/pwilkin/rocm-systems/tree/ilintar-experiments). That runtime is what makes HIP batched graph dispatch correct on `gfx1151`, and the build fails if the patch is missing from the tracked branch. Measured on Qwen3.8-Flash-Next Q4_K_XL with Unsloth's shared Q8 MTP head: 1207 t/s prompt processing (`pp2048`, depth 0), 1055 t/s at depth 32k, 43.9 t/s decode. Both sources are tracked branches, never pinned, so `--no-cache` is the only refresh. Manual build only; it compiles ROCm itself, so allow 40-60 minutes. Never set `GGML_CUDA_ENABLE_UNIFIED_MEMORY` on this image. See [local build steps, the measured launch flags, and the bisection switches](docs/building.md#retained-pm4-strix-halo-build-rocm-100-strix-llama). |
